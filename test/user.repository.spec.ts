...
describe('UserRepository', () => {
  ...
  describe('findById', () => {
    it('looks the user up by id and returns it', async () => {
      qbMock.getOne.mockResolvedValue(storedUser);

      const result = await repository.findById(1);

      expect(qbMock.where).toHaveBeenCalledWith('u.id = :id', { id: 1 });
      expect(result).toBe(storedUser);
    });
  ...
  describe('search', () => {
    it('applies no filters when none are provided', async () => {
      qbMock.getMany.mockResolvedValue([storedUser]);
    ...
    it('applies a case-insensitive partial match per provided filter', async () => {
      ...
      expect(qbMock.andWhere).toHaveBeenCalledWith('u.username ILIKE :username', {
        username: '%ad%',
      });
      expect(qbMock.andWhere).toHaveBeenCalledWith('u.email ILIKE :email', {
        email: '%example%',
      });
      expect(qbMock.andWhere).toHaveBeenCalledWith(
        'u.firstName ILIKE :firstName',
        { firstName: '%Ad%' },
      );
      expect(qbMock.andWhere).toHaveBeenCalledWith('u.lastName ILIKE :lastName', {
        lastName: '%Lov%',
      });
    });
  });
